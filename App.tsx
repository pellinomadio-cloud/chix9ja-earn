...
const App: React.FC = () => {
  ...
  const saveUserToStorage = (u: User) => {
    ...
    setDoc(doc(db, 'users', emailKey), sanitizedUser)
      .catch((error) => {
  ...
  const handleRegister = async (name: string, email: string, referredBy?: string) => {
    const defaultWelcomeBonus = 10000.00;
    const referralJoinedBonus = 2500.00;
      ...
      try {
        ...
        if (refSnap.exists()) {
          ...
          const updatedRef: User = {
            ...refData,
            balance: (refData.balance || 0) + 5000.00,
            referralCount: (refData.referralCount || 0) + 1,
            referralEarnings: (refData.referralEarnings || 0) + 5000.00,
            referredUsers: [...(refData.referredUsers || []), email.toLowerCase()],
    ...
    const newUser: User = {
      name, email, balance: startingBalance, isSubscribed: false,
      transactions,
